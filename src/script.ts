...
// Object containing cities and coordinates
// todo add some more cities
const cities = {
  stockholm: {
    name: "Stockholm",
    lat: 59.341952,
...
//Gets the wrapper element where we insert the UI components.
...
const metaBox = (result: any): HTMLElement => { 
  ...
  // TODO: Replace with real astronomical times
  let sunriseToday = "07:00";
  let sunsetToday = "20:00";

//Builds our meta information at the top of the page.
  div.innerHTML = `
    <ul class="meta-list">
...
const getConditionInfo = (code: number | string) => {
  ...
  if ([1, 2, 3, 4].includes(c)) {
    ...
    return{
      ...
      icon: `
      <img src="assets/img/icon-umbrella.svg" alt="Umbrella Icon" />
      `
    };
  }
}
...
const fetchWeatherAPI = async() => {
  try {
    ...
    console.log("En nivå in från response:", result.timeSeries[0].data); // The first weather point in the list.
    ...
    const { conditionText, icon } = conditionInfo;
    wrapper?.appendChild(metaBox(result));
    wrapper?.appendChild(conditionBox(conditionText, icon));
